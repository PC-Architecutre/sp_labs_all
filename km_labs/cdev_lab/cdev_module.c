...
static ssize_t my_read(struct file *filp, char __user *buf, size_t len, loff_t *                                                                                                             off)
{
        ...
        // функция для получния времени
        getnstimeofday(&t);
        // snprintf(data, BUF_LEN, "Cur time(sec: %i, mircosec: %i)", (int)t.tv_                                                                                                             sec, (int)t.tv_nsec);
        snprintf(data, BUF_LEN, "TIME: %.2lu:%.2lu:%.2lu:%.6lu \r\n",
                (t.tv_sec/3600)%(24)-4,
                (t.tv_sec/60)%(60),
                t.tv_sec%60,
                t.tv_nsec/1000);
